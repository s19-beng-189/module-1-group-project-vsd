...
%Duration of systole (minutes)
%CLV time constant during systole (minutes) %CLV time constant during diastole (minutes) %Systemic resistance (mmHg/(liter/minute)) %Pulmonary resistance (mmHg/(liter/minute))
Rp= 1.79
...
RPu=0.01 %pulmonic valve resistance (mmHg/(liter/minute)) %The following values of Csa and Cpa are approximate.
...
Vsvd=0 
Vpvd=0
%Max (diastolic) value of CRV (liters/mmHg) %Systemic arterial volume at P=O (liters) %Pulmonary arterial volume at P=O (liters) %Systemic venous volume at P=O (liters) %Pulmonary venous volume at P=O (liters) %Left ventricular volume at P=O (liters) %Right ventricular volume at P"'O (liters) %Time step duration (minutes)
VLVd=0.027
VRVd=0.027
dt=0.01*T
%This choice implies 100 timesteps per cardiac cycle. 
klokmax=15*T/dt %Total number of timesteps
%This choice implies simulation of 15 cardiac cycles. %Assign an index to each compliance vessel
...
%Note that the code that follows is independent %of the specific numbering scheme chosen above. 
%Compliance vector:
C=zeros (N, 1) ;
%This makes C a column vector of length N. C(iLV)=CV_now(O,CLVS,CLVD); %initial value C(isa)=Csa;
C(isv)=Csv;
...
%It drops out of the equations we solve for P, %but we need it if we want to output (e.g., plot) %the volume of any compliance vessel. 
Vd=zeros(N,1);
...
Vd(ipv)=Vpvd;
Vd
%This writes the results on the screen. %Conductance matrix:
G=zeros(N,N);
%This makes G an NxN matrix filled with zeros. %Any element of G that is not explicitly
%made nonzero remains zero,
%thus modeling an infinite resistance connection, %that is, no connection at all.
%But G(isa,iL V)=O (no leak) Y~o valve
%no valve
%But G(iRV,isv)=O; (no leak) %But G(ipa,iRV)=O; (no leak) %no valve
G(iLV ,isa)=1/RAo;
G(isa,isv)=1/Rs;
G(isv,isa)=1/Rs;
G(isv,iRV)=1/RTr;
...
%Other variables that we might want to plot %can be found from these.
%For self-checking in P_new, set CHECK=1.
%To skip self-checking set CHECK=O.

%(should be much faster with CHECK=O)
CHECK=0;
%Initialize flow computation (for output purposes only) %assign an index to each flow of interest:
jAo=1;
js =2;
jTr=3;
jPu=4;
...
%create arrays to store current pressure differences %and history over time of the net flows: 
